...
document.getElementById('cancelTambahSiswa').addEventListener('click', function () {
    document.getElementById('tambahSiswaModal').classList.add('hidden');
});
...
document.getElementById('tambahSiswaForm').addEventListener('submit', function (e) {
    ...
    // Ambil data form
    const nama = this.namasiswa.value.trim();
    const jenisKelamin = this.jenisKelamin.value;
    const nis = this.nis.value.trim();
    const kelas = this.kelas.value;
    const noHp = this.noHp.value.trim();
    const status = this.status.value;

    if (!nama || !jenisKelamin || !nis || !kelas || !noHp || !status) {
        alert('Semua field harus diisi!');
        return;
    }

    // Kirim data ke PHP pakai fetch
    fetch('siswa.php', {
        method: 'POST',
        headers: {
