...
document.getElementById("submit-reservation").addEventListener("click", async () => {
    ...
    const data = {
        nama_lengkap: identityForm["student-name"].value,
        nis: identityForm["student-nis"].value,
        kelas: identityForm["student-class"].value,
        no_telepon: identityForm["student-phone"].value,
        jenis_ruangan: reservationForm["room-type"].value,
        tanggal_peminjaman: reservationForm["reservation-date"].value,
        deskripsi_kegiatan: reservationForm["activity-description"].value,
        jam_mulai: reservationForm["start-time"].value,
        jam_selesai: reservationForm["end-time"].value,
        penanggung_jawab: reservationForm["responsible-teacher"].value
    };

    try {
        const response = await fetch("src/API/tu_peminjaman_ruangan.php", {
            method: "POST",
            headers: {
        ...
        // Baca response sebagai teks dulu
        ...
        try {
            ...
            alert("Response bukan JSON valid: " + e.message);
            return;
        }
...
async function loadHistory() {
    const historyBody = document.getElementById("reservation-history");
    historyBody.innerHTML = "<tr><td colspan='6' class='px-6 py-4 text-center text-gray-500'>Memuat...</td></tr>";

    try {
        const response = await fetch("src/API/tu_peminjaman_ruangan.php?action=list");
        
        // Baca response sebagai teks dulu
        const text = await response.text();
        console.log("Response text (history):", text);
        ...
        try {
            ...
            historyBody.innerHTML = "<tr><td colspan='6' class='px-6 py-4 text-center text-red-500'>Response bukan JSON valid</td></tr>";
            return;
        }
        ...
        if (result.success && result.data.length > 0) {
            ...
            result.data.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="px-6 py-4">${item.deskripsi_kegiatan}</td>
                    <td class="px-6 py-4">${item.status}</td>
                    <td class="px-6 py-4">
                        <button class="text-blue-600 hover:underline text-sm" onclick="alert('Fitur detail belum diaktifkan')">Detail</button>
                    </td>
                `;
                historyBody.appendChild(tr);
            });
        } else {
            historyBody.innerHTML = "<tr><td colspan='6' class='px-6 py-4 text-center text-gray-500'>Belum ada data.</td></tr>";
        }
    } catch (error) {
        console.error("Error loading history:", error);
        historyBody.innerHTML = "<tr><td colspan='6' class='px-6 py-4 text-center text-red-500'>Gagal memuat data.</td></tr>";
    }
}

// Tombol shortcut

// tombol "Lihat Riwayat" di halaman sukses
