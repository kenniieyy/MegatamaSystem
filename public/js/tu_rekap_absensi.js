...
async function fetchGuruData(tipe) {
  const res = await fetch(`tu_rekap_absensi.php?tipe=guru&absen=${tipe}`)
...
async function fetchSiswaData() {
  const res = await fetch('tu_rekap_absensi.php?tipe=siswa')
  return await res.json()
}
// //data dummy siswa
// const dummySiswa = [
...
document.getElementById("btn-export-pdf").addEventListener("click", () => {
  const tipe = document.getElementById("tipe").value
  window.open(`../src/API/export_absensi.php?tipe=${tipe}`, '_blank')
})
