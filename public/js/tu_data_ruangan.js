...
// ];
// >>>>>>> 6ea2a26beb334226e9bb3903c73dadb02db13627
...
//     try {
//       const response = await fetch('tu_data_ruangan.php', {
//         method: 'POST',
...
//     try {
//       const response = await fetch('tu_data_ruangan.php?action=get');
//       const result = await response.json();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  tambahRuanganForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showToast('Gagal', 'Terjadi kesalahan saat mengirim data.', 'error');
      console.error(error);
    }
  });
  ...
  function showToast(title, message, type = 'info') {
    ...
    toastNotification.classList.remove('hidden');
    if (type === 'success') {
      toastNotification.style.borderLeftColor = '#16a34a'; // hijau
    } else if (type === 'error') {
      toastNotification.style.borderLeftColor = '#dc2626'; // merah
    } else {
      toastNotification.style.borderLeftColor = '#2563eb'; // biru
    }
  }

  // Fungsi load data ruangan (contoh sederhana)
  async function loadDataRuangan() {
    try {
      const response = await fetch('tu_data_ruangan.php?action=get');
      const result = await response.json();
        ...
        result.data.forEach((ruangan, index) => {
          ruanganTableBody.innerHTML += `
            <tr>
              ...
              <td class="px-6 py-4">${ruangan.keterangan || '-'}</td>
              <td class="px-6 py-4">
                <!-- Tambahkan aksi edit dan hapus di sini jika perlu -->
                <button class="text-red-600 hover:underline" onclick="deleteRuangan(${ruangan.id})">Hapus</button>
              </td>
            </tr>
          `;
      ...
      ruanganTableBody.innerHTML = '<tr><td colspan="5" class="text-center py-4">Terjadi kesalahan.</td></tr>';
      console.error(error);
    }
  }
...
tambahRuanganForm.addEventListener('submit', async (e) => {
  ...
  const formData = new FormData(tambahRuanganForm);
  const data = Object.fromEntries(formData.entries());
  console.log('Submit data:', data);  // <--- ini
  // lanjut fetch...
});
